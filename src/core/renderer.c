...
void renderer_current_scene(Renderer *renderer) {
  ...
  if (curr->map != NULL) {
    for (int layer = 0; layer < curr_map->layer_count; layer++) {
      for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
          uint16_t gid = curr_map->layers[layer].gids[y * width + x];
          if (gid == 0) {
            continue;
          }
          ...
          Rectangle src = {tile_x * tile_width, tile_y * tile_height,
          ...
          if (has_camera) {
            ...
            if (CheckCollisionRecs(viewport, dest)) {
              ...
              for (int i = 0; i < curr->entity_count; i++) {
                DrawRectangleV(curr->entities[i].pos, curr->entities[i].size,
                               RED);
